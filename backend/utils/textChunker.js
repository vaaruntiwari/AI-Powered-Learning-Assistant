...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (currentWordCount + paragraphWordCount > chunkSize && currentChunk.length > 0) {
      ...
      // Handle overlap by keeping trailing words from the previous chunk
      const lastParagraph = currentChunk[currentChunk.length - 1];
      const lastWords = lastParagraph.split(/\s+/);
      if (lastWords.length > overlap) {
        const overlapWords = lastWords.slice(-overlap).join(' ');
  ...
  // Push remaining paragraphs as the final chunk
  if (currentChunk.length > 0) {
    chunks.push({
      content: currentChunk.join('\n\n'),
      chunkIndex: chunkIndex++,
