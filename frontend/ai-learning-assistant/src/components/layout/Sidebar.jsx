...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/80 z-50 flex flex-col justify-between p-4 shrink-0 md:static md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="px-3 py-4  border-t border-slate-200/80">
          ...
          >
            <LogOut size={18} strokeWidth={2.5} className="transition-transfor duration-200 group-hover:scale-110" />
            Logout
          </button>
        </div>
      </aside>
